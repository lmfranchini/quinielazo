...
try {
    ...
    $check = $db->prepare("SELECT id FROM `" . PREDICTION_TABLE . "` WHERE userId = ? AND matchId = ?");
    $check->execute([$userId, $matchId]);
    $existing = $check->fetch();

    if ($existing) {
        $upd = $db->prepare("UPDATE `" . PREDICTION_TABLE . "` SET scoreA = ?, scoreB = ?, updatedAt = NOW() WHERE userId = ? AND matchId = ?");
        $upd->execute([$scoreA, $scoreB, $userId, $matchId]);
    } else {
        $ins = $db->prepare("INSERT INTO `" . PREDICTION_TABLE . "` (userId, matchId, scoreA, scoreB, createdAt, updatedAt) VALUES (?, ?, ?, ?, NOW(), NOW())");
